...
Graphics Graphics::SubGraphics(const Rect &area) {
    return this->SubGraphics(area.Left, area.Top, area.Width, area.Height);
}

Graphics Graphics::SubGraphics(int16_t x, int16_t y, uint16_t width, uint16_t height) {
    x += this->x;
    y += this->y;
    int16_t w = MIN(this->left + this->width, x + width) - this->left;
    int16_t h = MIN(this->top + this->height, y + height) - this->top;
    const Point origin(x, y);
    const Point drawable(MAX(x, this->drawableX), MAX(y, this->drawableY));

    Graphics ret(Rect(this->left, this->top, w, h), this->data, this->dataWidth, origin, drawable);
    return ret;
}
    ...
    if(color.A == 0xFF) {
        ...
        if(p1.X == p2.X) {
            if((p1.X < 0) || (p1.X >= this->width))
                return;
            int16_t y1 = MAX(MIN(p1.Y, p2.Y), 0);
            int16_t y2 = MIN(MAX(p1.Y, p2.Y), this->height - 1);
            for(; y1 <= y2; y1++)
            ...
            if((p1.Y < 0) || (p1.Y >= this->height))
                return;
            int16_t x1 = MAX(MIN(p1.X, p2.X), 0);
            int16_t x2 = MIN(MAX(p1.X, p2.X), this->width - 1);
...
void Graphics::DrawRectangle(const Pen &pen, const Rect &rect) {
    ...
    int16_t x1 = MAX(absX, this->left, this->x, this->drawableX) - this->left;
    int16_t y1 = MAX(absY, this->top, this->y, this->drawableY) - this->top;
    int16_t x2 = MIN(absX + rect.Width, this->left + this->width) - this->left;
    int16_t y2 = MIN(absY + rect.Height, this->top + this->height) - this->top;
    int32_t temp;
    RgbColor *dataBuff = this->data;
...
void Graphics::DrawString(const char *text, const Font &font, const Color &color, const Rect &rect) {
    int16_t right = MIN(this->left + this->width, rect.Left + this->x + rect.Width);
    int16_t bottom = MIN(this->top + this->height, rect.Top + this->y + rect.Height);
